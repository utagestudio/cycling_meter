...
class CyclingDataManager:
    def __init__(self):
        self.data_file = './cycling_data.json'
        self.lock = threading.Lock()

    def save_data(self, speed, distance, elapsed_time, calories, cadence):
        logging.info("JSONに保存")
        data = {
            'speed': round(speed, 1),
...
def main ():
    ...
    data_manager = CyclingDataManager()
    ...
    try:
        ...
        while(True):
            ...
            if int(current_time) != int(last_time):
                ...
                speed = cadence * 4.45 * 60 / 1000
                draw.rectangle([(70,0),(105,15)], fill = 0xFF)
                draw.text((70,0), f"{speed:4.1f}", font = font15, fill = 0)
                ...
                draw.text((0,45), f"{calories:6.1f}", font = font40, fill = 0)
                epd.displayPartial(epd.getbuffer(image))

                # save
                data_manager.save_data(speed, distance, elapsed_display, calories, cadence)
